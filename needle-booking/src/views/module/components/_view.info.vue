...
<div class="example-info-container">
    ...
    <div>
      <el-form ref="ruleForm" label-width="150px" style="padding:15px 20px;">
        <el-row :gutter="0">
          ...
          <el-col v-if="!isAuditProp(prop.ruleType)"
            :key="prop.prop" :xs="24" 
            :sm="isShowInRow(prop)?24:12" style="padding-right:8%;">
            <el-form-item v-if="!isHideInInfo(prop.ruleType)" :label="prop.name">
              <div v-if="prop.feature==='IMAGE' && dataInfo[prop.prop]">
                <a href="#" @click="handlePreview(getFileUrl(dataInfo[prop.prop], prop.ruleType))">
                  <img :src="getFileUrl(dataInfo[prop.prop], prop.ruleType)" style="max-width:600px;" />
                </a>
              </div>
              <div v-else-if="prop.feature==='FILE' && dataInfo[prop.prop]">
                <a :href="getFileUrl(dataInfo[prop.prop], prop.ruleType)" target="_blank">
                  <i class="el-icon-document" />
                </a>
              </div>
              <div v-else-if="prop.feature==='EDITOR' && dataInfo[prop.prop]">
                <div v-html="dataInfo[prop.prop]"></div>
              </div>
        ...
        <el-row>
          ...
          <el-col v-if="isAuditProp(prop.ruleType)" :key="prop.prop"
            :xs="24" :sm="isShowInRow(prop)?24:12" style="padding-right:8%;">
            <el-form-item v-if="!isHideInInfo(prop.ruleType)" :label="prop.name">
              <div v-if="prop.feature==='IMAGE' && dataInfo[prop.prop]">
                <a href="#" @click="handlePreview(getFileUrl(dataInfo[prop.prop], prop.ruleType))">
                  <img :src="getFileUrl(dataInfo[prop.prop], prop.ruleType)" style="max-width:600px;" />
  ...
  <section v-for="child in this.module.children" :key="'_info_children_' + child.name">
    <Children ref="childrenList" :parent="module" :module="child" :key="'_children_'+child.name"></Children>
  </section>
</div>
</template>
...
import  { getEntity, remove, isHideInInfo, isAuditProp, isShowInRow, isAbsoluteFile} from '@/api/example'
export default {
  name: "info",
  components: { 
    Children
