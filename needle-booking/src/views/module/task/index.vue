  ...
  <div class="example-list-container">
    <div style="padding:15px 20px 5px 20px;">
      <el-form :inline="true">
        ...
        <el-form-item :label="filterProp.name" label-width="80px" style="text-align:right;"
      ...
      <div class="box">
        <div class="left">
          <template v-for="action in (module.actions || [])" >
            <el-button v-if="isSendTask && action.act === 'CREATE'" type="default" icon="el-icon-edit-outline"
              @click="handleCreate(action)" 
              :key="'tool_action_' + action.id"><span>{{ action.name }}</span>
            </el-button>
            <el-button v-else-if="isSendTask && action.act === 'UPDATE'" type="default" icon="el-icon-edit" 
              @click="handleUpdate(action)" 
              :key="'tool_action_' + action.id"><span>{{ action.name }}</span>
            </el-button>
            <el-button v-else-if="isSendTask && action.act === 'DELETE'" type="default" icon="el-icon-delete"  
              @click="handleDelete(action)" 
              :key="'tool_action_' + action.id"><span>{{ action.name }}</span>
            </el-button>
            <el-button v-else-if="isSendTask && action.act === 'IMPORT'" type="default" icon="el-icon-upload2" 
              @click="handleImport" :key="'tool_action_' + action.id">
              <span>{{action.name}}</span>
            </el-button>
            <el-button v-else-if="isSendTask && action.act==='EXPORT'" type="default" icon="el-icon-download" 
        ...
        <el-table v-loading="loading" @selection-change="handleSelectionChange" 
          ...
          <el-table-column label="操作" width="78">
            <template slot-scope="scope">
              ...
              <el-dropdown v-if="isSendTask && module.actions && module.actions.filter(a=>a.act==='UPDATE'||a.act==='DELETE').length>0" 
              ...
              <template slot-scope="scope">
                ...
                <div v-else>
                  <el-tag v-if="prop.prop === 'taskStatus'"
                    :type="scope.row[prop.prop]==='TODO'||scope.row[prop.prop]==='待处理'?'danger':'success'"
                    >{{ scope.row[prop.prop] }}
                  </el-tag>
                  <div v-else>{{ scope.row[prop.prop] }}</div>
                </div>
    ...
    <el-dialog :visible.sync="dialogInfoVisible" @open="openInfoDialog" 
      ...
      <span slot="footer" class="dialog-footer">
        <template v-for="action in (module.actions || [])">
          <el-button type="danger" v-if="isSendTask && action.act === 'UPDATE'" @click="handleInfoUpdate"
             :key="'info_footer_act_' + action.id">
            {{action.name}}
          </el-button>
          <el-button type="danger" v-if="isSendTask && action.act === 'DELETE'" @click="handleInfoDelete"
             :key="'info_footer_act_' + action.id">
            {{action.name}}
          </el-button>
        </template>
        <el-button type="danger" v-if="isSendTask && nextable" @click="handleTaskNext">下一步</el-button>
    ...
    <el-dialog :visible.sync="dialogNextVisible"
      :close-on-click-modal="false" 
      width="80%" :title="'下一步任务'">
      <Next ref="dialogNext" :module="module" @handleSuccess="handleTaskNextSuccess"></Next>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleTaskNextSave">保存</el-button>
        <el-button type="success" @click="dialogNextVisible=false">取消</el-button>
      </span>
    </el-dialog>
...
<script>
  ...
  import Next from './_task.next'

  import { Loading } from 'element-ui'
  import openWindow from '@/utils/open-window'
  import  { findList, remove, isHideInList, 
    isHideInInfo, columnMinWidth } from '@/api/example'
  ...
  export default {
    ...
    methods: {
      getViewFilters() {
        ...
        if(_id) {
          viewFilters.push({ prop: 'id', value: _id })
        }
      ...
      getList() {
        if(!this.loading) {
          ...
          params._sort = 'createdDate'
          params._sendTask = this.isSendTask
      ...
      handleTaskNext() {
        ...
        this.$nextTick(_ => {
          let params = { workFlow: this.workFlowId, prevTaskId: this.dataSelection.id, taskStatus:'TODO' }
          this.$refs.dialogNext.initialize(params)
        })
      },
      handleTaskNextSave() {
        this.$refs['dialogNext'].submitForm()
      },
      handleTaskNextSuccess() {
        this.getList()
