          ...
          <el-row :gutter="0">
            <el-col v-for="prop in moduleProps" :key="prop.prop" style="padding-right:10%;"
              ...
              <template v-if="!isHideInEdit(prop.ruleType)">
                <el-form-item v-if="prop.prop==='province'" :label="prop.name" :prop="prop.prop">
                  <FormFK :moduleProp="prop" v-model="dataForm[prop.prop]" :showValue="dataForm[prop.showPath]" :defaultFilters="[{prop:'parent',value:1}]" :readonly="isNoUpdate(prop.ruleType)" @handleBack="(mappings, data)=>{Object.keys(mappings).forEach(refProp => {let prop=mappings[refProp]; $set(dataForm, prop, data[refProp]); setDefaultProvinceFilters(dataForm.province) })}"></FormFK> 
                </el-form-item>
                <el-form-item v-else-if="prop.prop==='city'" :label="prop.name" :prop="prop.prop">
                  <FormFK :moduleProp="prop" v-model="dataForm[prop.prop]" :showValue="dataForm[prop.showPath]" :defaultFilters="defaultProvinceFilters" :refreshBeforeShow="true" :readonly="isNoUpdate(prop.ruleType)" @handleBack="(mappings, data)=>{Object.keys(mappings).forEach(refProp => {let prop=mappings[refProp]; $set(dataForm, prop, data[refProp]); setDefaultCityFilters(dataForm.city) })}"></FormFK> 
                </el-form-item>
                <el-form-item v-else-if="prop.prop==='district'" :label="prop.name" :prop="prop.prop">
                  <FormFK :moduleProp="prop" v-model="dataForm[prop.prop]" :showValue="dataForm[prop.showPath]" :defaultFilters="defaultCityFilters" :refreshBeforeShow="true" :readonly="isNoUpdate(prop.ruleType)" @handleBack="(mappings, data)=>{Object.keys(mappings).forEach(refProp => {let prop=mappings[refProp]; $set(dataForm, prop, data[refProp]); })}"></FormFK> 
                </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      defaultProvinceFilters: [],
      defaultCityFilters: [],

      props: {
        lazy: true,
        lazyLoad (node, resolve) {
          const parentId = node.value || 1
          let viewFilter = { "prop": 'parent', "op":"=", "value": parentId }
          let viewFilters = []
          viewFilters.push(viewFilter)
          findList('region', { '_vf' : JSON.stringify(viewFilters) , _size: 1000, _sort:'name'}).then(response => {
  ...
  methods: {
    ...
    setDefaultProvinceFilters(parent) {
      this.defaultProvinceFilters = [{prop: 'parent', value: parent}]
    },
    setDefaultCityFilters(parent) {
      this.defaultCityFilters = [{prop: 'parent', value: parent}]
    },
    getInfo(data) {
      getEntity(this.module.name, data[this.module.pk]).then(response => {
        this.dataForm = response.data
        this.defaultProvinceFilters = [{prop: 'parent', value: this.dataForm.province}]
        this.defaultCityFilters = [{prop: 'parent', value: this.dataForm.city}]
      }).catch(() => {})
    },
    getChildren() {
