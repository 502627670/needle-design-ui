              ...
              <el-col :key="prop.prop" :xs="24" style="padding-right:10%;"
                ...
                <template v-if="!isHideInForm(prop.ruleType)">
                  <el-form-item v-if="prop.prop==='province'" :label="prop.name" :prop="prop.prop">
                    <el-cascader v-model="dataForm[prop.prop]" :props="props" style="width:100%;"></el-cascader>
                  </el-form-item>
                  <el-form-item v-else-if="prop.prop!='city' && prop.prop!='district'" :label="prop.name" :prop="prop.prop">
                    <el-input v-if="prop.feature==='STRING'" :moduleProp="prop" v-model="dataForm[prop.prop]" autocomplete="off" :maxlength="prop.length>0?prop.length:60" @input="($event)=>{$forceUpdate()}"></el-input>
                    ...
                    <FormSelect v-else-if="prop.feature==='SELECT'" :moduleProp="prop" v-model="dataForm[prop.prop]" :showValue="dataForm[prop.prop]"></FormSelect>
...
import  { findList, create, isRequired, isID, isHideInForm, isShowInRow } from '@/api/example'
...
export default {
  ...
  methods: {
    ...
    getActionDataList() {
      let provinceList = this.dataForm['province']
      if(provinceList) {
        provinceList = provinceList.toString()
        let parray = provinceList.split(',')
        this.dataForm.province = parray[0]
        this.dataForm.city = parray[1]
        this.dataForm.district = parray[2]
      }
