      ...
      <div>
        <el-form ref="ruleForm" :model="dataForm" :key="dataFormKey" :rules="rules" label-width="160px">
          <el-row :gutter="0">
            ...
            <el-col :sm="12">
              <el-form-item label="业务项目" prop="module">
            ...
            <el-col :sm="12">
              <el-form-item label="业务数据" prop="instanceId">
                <FormFK ref="instanceId" v-model="dataForm['instanceId']" 
                  :moduleProp="instanceModuleProp" 
                  :showValue="dataForm['instanceTitle']" 
                  :readonly="Object.keys(instanceModuleProp).length<=0">
                </FormFK>
              </el-form-item>
            </el-col>
            <el-col :sm="12">
              <el-form-item label="接收人" prop="assigneeUser">
                <FormFK ref="assignee" 
                  ...
                  :showValue="dataForm['assigneeUser.username']"></FormFK> 
              </el-form-item>
            </el-col>
            <el-col :sm="12">
              <el-form-item label="任务状态">
                <el-select v-model="dataForm.taskStatus" style="width:100%;">
                  <template v-for="(item,index) in getTaskStatusPropValues()">
                    <el-option 
                      :label="item.label" :value="item.value" 
                      :key="'mp_task_status_'+index">
                    </el-option>
                  </template>
                </el-select>
              </el-form-item>
...
import  { findTaskModules, getTaskModule, getEntity, update, isRequired,
...
export default {
  ...
  methods: {
    ...
    getTaskStatusPropValues() {
      let mp = this.moduleProps.find(mp => mp.prop === 'taskStatus') || {}
      return mp.values || []
    },
    ...
    submitForm() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          let params = {subtitle: this.dataForm.subtitle, module: this.dataForm.module}
          params.id = this.dataForm.id
          params.instancePk = this.module.pk
          params.instanceId = this.dataForm.instanceId
          params.instanceTitle = this.$refs.instanceId.getContent()
          params.assignee = this.$refs.assignee.getContent()
          params.taskStatus = this.dataForm.taskStatus
