    ...
    <div  class="example-info-body" style="border:0px solid #d2d6de;">
      <div>
        <el-form ref="ruleForm" :model="dataForm" :key="dataFormKey" :rules="rules" label-width="160px">
          <el-row :gutter="0">
            <template v-for="prop in moduleProps">
              <el-col :key="prop.prop" :xs="24" style="padding-right:10%;"
                :sm="isShowInRow(prop)?24:12" class="form-item-col">
                <template v-if="!isHideInForm(prop.ruleType)">
                  <el-form-item v-if="prop.prop === 'module'" :label="prop.name" :prop="prop.prop">
                  ...
                  <el-form-item v-else-if="prop.prop === 'prop'" :label="prop.name" :prop="prop.prop">
                    <el-select v-model="dataForm.prop" style="width:100%;" clearable>
                      <template v-for="(prop, index) in selectedTaskModule.props">
                        <el-option v-if="!isID(prop.ruleType)&&!isAuditProp(prop.ruleType)&&!isSystemProp(prop.ruleType)" 
                          :key="'mdx_children_' + index" 
                          :label="prop.name" :value="prop.prop">
                        </el-option>
                      </template>
                    </el-select>
                  </el-form-item>
                  <el-form-item v-else :label="prop.name" :prop="prop.prop">
                    <el-input v-if="prop.feature==='STRING'" :moduleProp="prop" v-model="dataForm[prop.prop]" autocomplete="off" :maxlength="prop.length>0?prop.length:60" @input="($event)=>{this.$forceUpdate()}"></el-input>
                    ...
                    <FormFK v-else-if="prop.feature==='FK'" :moduleProp="prop" v-model="dataForm[prop.prop]" :showValue="dataForm[prop.showPath]" @handleBack="(mappings, data)=>{Object.keys(mappings).forEach(refProp => {let prop=mappings[refProp]; $set(dataForm, prop, data[refProp]); })}"></FormFK> 
...
import  { getTaskModule, findTaskModules, create, isRequired, isID, 
  isAuditProp, isSystemProp, isHideInForm, isShowInRow } from '@/api/example'
export default {
  name: "froms",
  components: {
    FormString, 
  ...
  methods: {
    ...
    submitForm() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          let selectModule = this.dataForm.module
          let selectProp = this.dataForm.prop
          if(selectModule) {
            let selectModuleName = this.taskModules.find(m => m.name === selectModule).showName
            let selectProp = this.selectedTaskModule.props.find(mp => mp.prop === selectProp)
            let selectPropName = selectProp ? selectProp.name : ''
            this.dataForm.moduleName = selectModuleName
            this.dataForm.propName = selectPropName
          }
    ...
    handleTaskModuleChange(val) {
      ...
      if(val) {
        getTaskModule(val, {module: val}).then(response => {
          this.selectedTaskModule = response.data
        }).catch(() => {})
      }
      this.$set(this.dataForm, 'prop', '')
    },
    isShowInRow,
    isID,
    isAuditProp,
